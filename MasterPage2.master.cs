...
public partial class MasterPage2 : System.Web.UI.MasterPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Global.LogInDone)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    ddLicense.Items.Add(new ListItem("Select Company", "-1"));
            ...
            divContactEmail.Visible = false;
            divContactNumbers.Visible = false;
            divAdmin.Visible = true;

            if (Global.CurrentUserType == UserTypes.UT_Admin || Global.CurrentUserType == UserTypes.UT_AgentAdmin)
                ltAdminLink.Text = "<a href='/adminpanel' class='btn btn-info btn-sm' style='height: 20px;margin-top: 0px;padding: 0px;'>Admin</a>";
            ...
            if (Global.CurrentUserType == UserTypes.UT_Customer)
                ...
                ltAdminLink.Text = "<a href='/customer' class='btn btn-info btn-sm' style='height: 20px;margin-top: 0px;padding: 0px;'>My Account</a>";

            ltLoginText.Text = "<a href='/account/login.aspx?action=logout'  class='label label-info' style='font-size:small;'>Logout</a>";
        }

        else
        {
            ltLoginText.Text = "<a href='/account/login.aspx'  class='label label-info' style='font-size:small;'>Login</a>";
        }
    }
    protected void FillYearDropDown()
    {
        ...
        if (Global.FinancialYearID == -1)
        {
            ...
            ddFinancialYear.SelectedValue = Global.FinancialYearID.ToString();
    }
