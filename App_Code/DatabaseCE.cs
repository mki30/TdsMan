...
public class DatabaseCE
    {
        ...
        public DatabaseCE(string strAccessConn = "")
        {
            if (string.IsNullOrWhiteSpace(strAccessConn))
                strAccessConn = @"Data Source=|DataDirectory|\TDS.sdf;";
        ...
        public DataSet GetDataSet(string sqlQuery, ref string vError)
        {
            ...
            if(myconnection.ConnectionString.ToLower().Contains(".sdf"))
                dbAdapter = new SqlCeDataAdapter(sqlQuery, myconnection.ConnectionString);
            else
                dbAdapter = new OleDbDataAdapter(sqlQuery, myconnection.ConnectionString);
